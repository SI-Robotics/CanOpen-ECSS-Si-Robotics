...
if [[ ! -d "${SOURCE_DIR}" ]]; then
  ...
  ( cd "${SOURCE_DIR}" && git fetch --depth=1 origin "${REPO_BRANCH}" && git checkout -f "${REPO_BRANCH}" )
else
...
# Tools check
echo "Checking tools..."
for tool in git autoreconf autoconf automake libtool make arm-none-eabi-gcc arm-none-eabi-ar arm-none-eabi-ranlib; do
...
# Always run autoreconf -i (as requested)
echo "Running autoreconf -i..."
autoreconf -i "${SOURCE_DIR}"

# Flags
CPUFLAGS="-ffreestanding -mlittle-endian -mthumb -mcpu=${CPU} -ffunction-sections"
FPFLAGS=""
if [[ -n "${FPU}" ]]; then
  FPFLAGS="-mfloat-abi=${FLOAT_ABI} -mfpu=${FPU}"
else
  # No FPU: ensure soft float is used
  FPFLAGS="-mfloat-abi=soft"
fi

# NOTE: Keep your ECSS-specific defs like in the original script
CFLAGS="-DHAVE_CONFIG_H -DCO_DEFTYPE_TIME_SCET=0x0060 -DCO_DEFTYPE_TIME_SUTC=0x0061 -DLELY_HAVE_ITIMERSPEC=1 ${CPUFLAGS} ${FPFLAGS} -ggdb3"
LDFLAGS="${CPUFLAGS} ${FPFLAGS} -Wl,--gc-sections -lnosys --specs=nosys.specs"
...
# Copy config.h like in your CM7 script
